...
import { X, CheckCircle2, CreditCard, Bitcoin, Lock, Loader2, ArrowLeft, Copy, QrCode } from "lucide-react";
...
export default function PaywallModal({ onClose, user }) {
  ...
  const [copiedField, setCopiedField] = useState(null); // 'address' or 'amount'
    ...
    <div style={overlayStyle}>
      <div style={modalStyle}>
        ...
        {!verifying && verifyStatus !== "success" && (
          selectedMethod ? (
            <button onClick={resetState} style={backButtonStyle}>
        ...
        {/* ---------------------------
            STATE 2: PACKAGE SELECTION (Shared by both)
            --------------------------- */}
          ...
          <div style={transferDetailsBox}>
            ...
            <div style={infoBoxStyle}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid rgba(255,255,255,0.05)", paddingBottom: "10px", marginBottom: "10px" }}>
                <div>
                  <div style={{fontSize: "12px", color: "#8e8e93"}}>AMOUNT TO SEND</div>
                  <div style={{fontSize: "20px", fontWeight: "bold", color: "#fff"}}>
                    {cryptoPaymentDetails.pay_amount} <span style={{fontSize: "14px", color: "#f7931a"}}>{cryptoPaymentDetails.pay_currency.toUpperCase()}</span>
                  </div>
                </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ overflow: "hidden" }}>
                  <div style={{fontSize: "12px", color: "#8e8e93"}}>WALLET ADDRESS</div>
                  <div style={{fontSize: "13px", color: "#fff", wordBreak: "break-all", fontFamily: "monospace", marginTop: "3px"}}>
                    {cryptoPaymentDetails.pay_address}
                  </div>
                </div>
                <button onClick={() => copyToClipboard(cryptoPaymentDetails.pay_address, 'address')} style={{...copyBtnStyle, marginLeft: "10px"}}>
                  {copiedField === 'address' ? <CheckCircle2 size={16} color="#34C759"/> : <Copy size={16} color="#fff"/>}
                </button>
              </div>
            </div>

            {/* Zero-dependency QR Code generated via free API */}
            <div style={{ display: "flex", justifyContent: "center", margin: "20px 0" }}>
               <img 
                 src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${cryptoPaymentDetails.pay_address}&bgcolor=ffffff`} 
                 alt="Crypto QR Code" 
                 style={{ borderRadius: "10px", padding: "5px", background: "#fff" }}
               />
            </div>

            <div style={{ background: "rgba(247, 147, 26, 0.1)", border: "1px solid rgba(247, 147, 26, 0.2)", padding: "12px", borderRadius: "10px", fontSize: "13px", color: "#f7931a", textAlign: "center", lineHeight: "1.4" }}>
               Crypto transactions take 5 to 15 minutes to confirm on the blockchain. Once sent, you can close this window. Your account will automatically upgrade!
            </div>
          </div>
        )}

        {/* ---------------------------
            STATE 4: FIAT POLLING FEEDBACK
            --------------------------- */}
        {verifying && (
          <div style={{ textAlign: "center", padding: "40px 10px" }}>
