...
export default function Profile({ user, onLogout, setHideFooter, setActiveVideo }) {
  ...
  // 🟢 1. UI & SCROLL STABILITY
  // We handle Settings view footer visibility here.
  // The Video Player visibility is handled by App.jsx.
  useEffect(() => {
    ...
    // Cleanup ensures navigation away from Profile resets the footer
  ...
  const handleOpenVideo = useCallback(async (video, e) => {
    ...
    try {
      // Signal App.jsx to open the global player (shows shell + loader)
...
// 🖌 STYLES
const containerStyle = { minHeight: "100%", background: "var(--bg-color)", color: "#fff", position: "relative" };
